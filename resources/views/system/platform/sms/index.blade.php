    ...
    <script type="application/javascript">
        layui.use(['table'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('/system/platform/sms/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '短信记录列表'
                ...
                , cols: [[
                    ...
                    , {field: 'type_cn', title: '短信类型', align: 'center', width: 120}
                    , {field: 'phone', title: '接收手机', align: 'center', width: 150}
                    , {field: 'content', title: '短信内容'}
                    , {field: 'created_at', title: '发送时间', sort: true, align: 'center', width: 160}
                    , {
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '新增系统消息'
                            , content: '{{url('system/platform/sms/create')}}'
                            , maxmin: true
                            , area: ['600px', '400px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //查看
                    ...
                    var p = layer.confirm('确定要删除该条数据？', function (index) {
                        layer.close(p);
                        layer.load();
                        axios.post("{{url('system/platform/sms/destroy')}}", {ids: [data.id]})
