    ...
    <script type="application/javascript">
        layui.use(['table'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('/system/platform/banner/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '礼物列表'
                ...
                , cols: [[
                    ...
                    , {
                        field: 'url', title: '效果预览', align: 'center',
                        templet: function (d) {
                            // let html = '<a href="' + d.url + '" target="_blank"><img width="200px" height="50px" src="' + d.url + '" alt=""/></a>';
                            // return html;
                            return '<div onclick="show_img(this)" ><img src="' + d.url + '" alt="" style="height: 46px;" ></a></div>';
                        }
                    }
                    , {
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '新增Banner图'
                            , content: '{{url('system/platform/banner/create')}}'
                            , maxmin: true
                            , area: ['800px', '400px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //查看
                    ...
                    var p = layer.confirm('确定要删除该条数据？', function (index) {
                        layer.close(p);
                        layer.load();
                        axios.post("{{url('system/platform/banner/destroy')}}", {ids: [data.id]})
                    ...
                    layer.open({
                        type: 2
                        , title: '编辑Banner图'
                        , content: '{{url('system/platform/banner/edit?id=')}}' + data.id
                        , maxmin: true
                        , area: ['800px', '400px']
                        , end: function () {
                            tableIns.reload();
                    ...
                    var p = layer.confirm('确定' + str + '该条数据？', function (index) {
                        layer.close(p);
                        layer.load();
                        axios.post("{{url('system/platform/banner/status')}}", {id: data.id})
                            .then(function (response) {
                                    layer.closeAll();
                                    if (response.data.status) {
                                        layer.msg(response.data.msg);
        ...
        function show_img(t) {
            ...
            layer.open({
                ...
                content: '<div style="text-align:center;height: 100%;"><img src="' + $(t).attr('src') + ' " height="100%" width="100%"/></div>'
            });
        }
    </script>
@endsection
