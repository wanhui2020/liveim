    ...
    <script type="application/javascript">
        layui.use(['table'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('/system/platform/file/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '平台文件'
                ...
                , cols: [[
                    ...
                    {field: 'yulan', title: '预览', align: 'center'}
                    , {field: 'url', title: '存储路径', align: 'center'}
                    , {field: 'suffix', title: '文件类型', align: 'center'}
                    , {
                        field: 'place', title: '存储引擎', align: 'center',
                        templet: function (d) {
                            let text = '本地';
                            if (d.place === 1) {
                                text = '阿里云';
                    ...
                    , {field: 'size', title: '大小(KB)', align: 'center'}
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '新增平台文件'
                            , content: '{{url('system/platform/file/create')}}'
                            , maxmin: true
                            , area: ['600px', '400px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //查看
                    ...
                    var p = layer.confirm('确定要删除该条数据？', function (index) {
                        layer.close(p);
                        layer.load();
                        axios.post("{{url('system/platform/file/destroy')}}", {ids: [data.id]})
                    ...
                    layer.open({
                        type: 2
                        , title: '编辑基础数据'
                        , content: '{{url('system/platform/file/edit?id=')}}' + data.id
                        , maxmin: true
                        , area: ['800px', '400px']
                        , end: function () {
                            tableIns.reload();
