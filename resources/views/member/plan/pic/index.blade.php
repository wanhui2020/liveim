    ...
    <script type="application/javascript">
        ...
        layui.use(['table'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('/system/member/plan/pic/lists?planid=')}}' + planid //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '商务计划图片列表'
                ...
                , cols: [[
                    ...
                    , {
                        field: 'url', title: '图片预览', align: 'center',
                        templet: function (d) {
                            let html = '<a href="' + d.pic + '" target="_blank"><img width="400px" height="100px" src="' + d.pic + '" alt=""/></a>';
                            return html;
                        }
                    }
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '新增商务计划图片'
                            , content: '{{url('system/member/plan/pic/create')}}'
                            , maxmin: true
                            , area: ['800px', '400px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //查看
                    ...
                    var p = layer.confirm('确定要删除该条数据？', function (index) {
                        layer.close(p);
                        layer.load();
                        axios.post("{{url('system/member/plan/pic/destroy')}}", {ids: [data.id]})
