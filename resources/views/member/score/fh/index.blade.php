    ...
    <script type="application/javascript">
        layui.use(['table', 'laydate'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('/system/member/score/fh/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '富豪积分列表'
                ...
                , cols: [[
                    ...
                    , {field: 'score', title: '积分数量', align: 'center', width: 120}
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '新增富豪积分'
                            , content: '{{url('system/member/score/fh/create')}}'
                            , maxmin: true
                            , area: ['600px', '600px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //查看
                    ...
                    var p = layer.confirm('确定要删除该条数据？' + data.id, function (index) {
                        layer.close(p);
                        layer.load();
                        axios.post("{{url('system/member/score/fh/destroy')}}", {ids: [data.id]})
