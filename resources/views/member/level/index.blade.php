    ...
    <script type="application/javascript">
        layui.use(['table', 'laydate'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('/system/member/level/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '会员等级'
                ...
                , cols: [[
                    ...
                    {field: 'lvl', title: '等级', align: 'center', sort: true, width: 80}
                    , {field: 'name', title: '名称', align: 'center'}
                    , {
                        field: 'fanwei', title: '等级条件范围', align: 'center',
                        templet: function (d) {
                            let html = '分值<=' + d.max_score;
                            return html;
                        }
                    }
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '新增会员等级'
                            , content: '{{url('system/member/level/create')}}'
                            , maxmin: true
                            , area: ['800px', '400px']
                            , end: function () {
                                tableIns.reload();
            ...
            table.on('tool(lists)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //查看
                    ...
                    var p = layer.confirm('确定要删除会员等级' + data.name + '？', function (index) {
                        layer.close(p);
                        layer.load();
                        axios.post("{{url('system/member/level/destroy')}}", {ids: [data.id]})
                    ...
                    layer.open({
                        type: 2
                        , title: '编辑会员等级'
                        , content: '{{url('system/member/level/edit?id=')}}' + data.id
                        , maxmin: true
                        , area: ['800px', '400px']
                        , end: function () {
                            tableIns.reload();
                    ...
                    var p = layer.confirm('确定' + str + '会员等级' + data.name + '？', function (index) {
                        layer.close(p);
                        layer.load();
                        axios.post("{{url('system/member/level/status')}}", {id: data.id})
                            .then(function (response) {
                                    layer.closeAll();
                                    if (response.data.status) {
                                        layer.msg(response.data.msg);
