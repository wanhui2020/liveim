    ...
    <script type="application/javascript">
        layui.use(['table'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('/system/member/credit/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '信誉评价'
                ...
                , cols: [[
                    ...
                    , {
                        field: 'tomember', title: '评价对象', align: 'center', templet: function (d) {
                            if (d.tomember == null) {
                                return '-';
                            }
                    ...
                    , {field: 'xy_score', title: '信誉评分', width: 120, align: 'center'}
                    , {field: 'yz_score', title: '颜值评分', width: 120, align: 'center'}
                    , {field: 'xy_score', title: '满意度评分', width: 120, align: 'center'}
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '新增会员评分'
                            , content: '{{url('system/member/credit/create')}}'
                            , maxmin: true
                            , fixed: false //不固定
                            , area: ['600px', '400px']
                            , end: function () {
            ...
            table.on('tool(lists)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //查看
                    ...
                    var p = layer.confirm('确定要删除该记录？', function (index) {
                        layer.close(p);
                        layer.load();
                        axios.post("{{url('system/member/credit/destroy')}}", {ids: [data.id]})
