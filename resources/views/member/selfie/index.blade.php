    ...
    <script type="application/javascript">
        layui.use(['table', 'laydate'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('/system/member/selfie/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '会员自拍认证'
                ...
                , cols: [[
                    ...
                    , {
                        field: 'pic', title: '自拍照', align: 'center', width: 150, templet: function (d) {
                            ...
                            // return html;
                            return '<div onclick="show_img(this)" ><img src="' + d.pic + '" alt="" width="500px" height="80px"></a></div>';
                        }
                    }
                    , {
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '新增自拍认证'
                            , content: '{{url('system/member/selfie/create')}}'
                            , maxmin: true
                            , fixed: false //不固定
                            , area: ['600px', '400px']
                            , end: function () {
            ...
            table.on('tool(lists)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //查看
                    ...
                    var p = layer.confirm('确定要删除该记录？', function (index) {
                        layer.close(p);
                        layer.load();
                        axios.post("{{url('system/member/selfie/destroy')}}", {ids: [data.id]})
                    ...
                    location.href='{{url('system/member/selfie/deal?id=')}}' + data.id;

                {{--layer.open({--}}
                    {{--    type: 2--}}
                    {{--    , title: '自拍认证审核'--}}
                    {{--    , content: '{{url('system/member/selfie/deal?id=')}}' + data.id--}}
                    {{--    , maxmin: true--}}
                    {{--    , area: ['600px', '400px']--}}
                    {{--    , end: function () {--}}
                    {{--        tableIns.reload();--}}
