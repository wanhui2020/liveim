    ...
    <script type="application/javascript">
        layui.use(['table', 'laydate'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('/system/member/gift/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '赠送礼物'
                ...
                , cols: [[
                    ...
                    , {
                        field: 'member', title: '赠送会员', align: 'center', width: 180, templet: function (d) {
                            if (d.member == null) {
                                return '-';
                            }
                    ...
                    , {field: 'quantity', title: '礼物数量', width: 100, align: 'center'}
                    , {field: 'gold', title: '总金币', width: 100, align: 'center', sort: true}
                    , {
                        field: 'is_sys', title: '后台赠送', align: 'center', width: 100,
                        templet: function (d) {
                            let color = 'green';
                            if (d.is_sys === 0) {
                                return '<a>否</a>';
                            }
                            return '<a style="color: green">是</a>';
                        }
                    }
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '赠送礼物'
                            , content: '{{url('system/member/gift/create')}}'
                            , maxmin: true
                            , fixed: false //不固定
                            , area: ['600px', '400px']
                            , end: function () {
            ...
            table.on('tool(lists)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //查看
                    ...
                    var p = layer.confirm('确定要删除该记录？', function (index) {
                        layer.close(p);
                        layer.load();
                        axios.post("{{url('system/member/gift/destroy')}}", {ids: [data.id]})
