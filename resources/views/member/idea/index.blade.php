    ...
    <script type="application/javascript">
        layui.use(['table', 'laydate'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('/system/member/idea/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '意见反馈'
                ...
                , cols: [[
                    ...
                    , {
                        field: 'member', title: '会员', align: 'center', width: 120, templet: function (d) {
                            if (d.member == null) {
                                return '-';
                            }
                    ...
                    , {field: 'replay', title: '回复'}
                    , {
                        field: 'deal_user', title: '回复人/时间', align: 'center', width: 160, templet: function (d) {
                            if (d.status == 0) {
                                return '-'
                            }
                            let html = d.replay_user + '<br>' + d.replay_time;
                            return html;
                        }
                    }
                    , {
                    ...
                    , {
                        title: '操作', width: 120, templet: function (d) {
                            let html = '';
                            if (d.status == 0) {
                                html += '<a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="deal">回复</a>';
                            }
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '新增意见反馈'
                            , content: '{{url('system/member/idea/create')}}'
                            , maxmin: true
                            , fixed: false //不固定
                            , area: ['600px', '400px']
                            , end: function () {
            ...
            table.on('tool(lists)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //查看
                    ...
                    var p = layer.confirm('确定要删除该记录？', function (index) {
                        layer.close(p);
                        layer.load();
                        axios.post("{{url('system/member/idea/destroy')}}", {ids: [data.id]})
                    ...
                    layer.open({
                        type: 2
                        , title: '意见回复'
                        , content: '{{url('system/member/idea/deal?id=')}}' + data.id
                        , maxmin: true
                        , area: ['600px', '400px']
                        , end: function () {
                            tableIns.reload();
