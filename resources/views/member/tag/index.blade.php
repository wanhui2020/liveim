        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto" lay-filter="aa">
                <div class="layui-form-item">
                    <div class="layui-inline" style="margin-right: -11px;">
                        <div class="layui-input-inline">
                            <input type="text" name="key" placeholder="输入会员关键字" autocomplete="off" class="layui-input">
                        </div>
                    </div>
                    <div class="layui-inline">
                        <div class="layui-input-inline">
                            <input type="text" name="tag" placeholder="输入分类标签关键字" autocomplete="off"
                                   class="layui-input">
                        </div>
                    </div>
                    <div class="layui-inline">
    ...
    <script type="application/javascript">
        layui.use(['table', 'laydate'], function () {
            ...
            let tableIns = table.render({
                elem: '#lists'
                , url: '{{url('/system/member/tag/lists')}}' //数据接口
                , method: 'POST'
                , toolbar: '#toolbar'
                , page: true //开启分页
                , title: '会员分类标签'
                ...
                , cols: [[
                    ...
                    , {
                        field: 'member', title: '主播', align: 'center', templet: function (d) {
                            if (d.member == null) {
                                return '-';
                            }
                    ...
                    , {
                        field: 'tag_cn', title: '分类标签', align: 'center', templet: function (d) {
                            if (d.tag == null) {
                                return '-';
                            }
                            return d.tag.name;
                    ...
                    , {
                        field: 'sort', title: '分类排序', align: 'center', width: 100
                    }
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            type: 2
                            , title: '设置主播分类标签'
                            , content: '{{url('system/member/tag/create')}}'
                            , maxmin: true
                            , fixed: false //不固定
                            , area: ['600px', '400px']
                            , end: function () {
            ...
            table.on('tool(lists)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //查看
                    ...
                    layer.open({
                        type: 2
                        , title: '编辑排序'
                        , content: '{{url('system/member/tag/edit?id=')}}' + data.id
                        , maxmin: true
                        , area: ['400px', '400px']
                        , end: function () {
                            tableIns.reload();
                    ...
                    var p = layer.confirm('确定要删除该记录？', function (index) {
                        layer.close(p);
                        layer.load();
                        axios.post("{{url('system/member/tag/destroy')}}", {ids: [data.id]})
